...
class UserRepository:
    ...
    def all(self) -> list[User]:
        ...
        users = []
        for row in rows:
            user = User(row['id'], row['email'], row['password'], row['handle'], row['name'],
                        row['joined_on'])
            users.append(user)
        return users
    ...
    def find(self, user_id:int) -> User:
        ...
        row = rows[0]
        return User(row['id'], row['email'], row['password'], row['handle'], row['name'],
                        row['joined_on'])

    # Find a single user_id by username 
    def find_id_by_handle(self, handle:str) -> int or None:
        rows = self._connection.execute('SELECT id FROM users WHERE handle = %s', [handle])
        user_id = rows[0]['id'] 
    ...
    def create(self, user:User) -> int:
        ...
        rows = self._connection.execute('INSERT INTO users (email, password, handle, name, joined_on) VALUES (%s, %s, %s, %s, %s) RETURNING id',
