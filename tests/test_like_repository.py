...
def test_like(db_connection):
    ...
    assert repository.who_liked_this(3, None) == [3, 1]
...
def test_unlike(db_connection):
    ...
    assert repository.who_liked_this(1, None) == []

def test_who_liked_this(db_connection):
    '''
    We can see a list of all user_id's for users that liked the post in question
    '''
    db_connection.seed("seeds/chwitter.sql")
    repository = LikeRepository(db_connection)
    assert repository.who_liked_this(post_id=2) == [2,3] #TODO Change this to User once User Repo is complete
    assert repository.who_liked_this(comment_id=2) == []
